  ...
  return (
    ...
    >
      <div className="aspect-w-16 aspect-h-9">
        <img src={template.preview_url || ''} alt={template.title || template.id} className="w-full h-40 object-cover" />
      </div>
      <div className="p-4 flex flex-col flex-grow">
        <div className="flex-grow">
            ...
            <h3 className="font-bold text-slate-900 dark:text-white truncate">{template.title || template.id}</h3>
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-between items-baseline">
            <p className="text-lg font-bold text-slate-800 dark:text-white">₹{(template.price ?? 0).toFixed(2)}</p>
